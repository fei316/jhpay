 ...
 */
class QQH5Controller extends PayController
{
    ...
    public function Pay($channel)
    {
        ...
        $params["trade_type"] = "JSAPI"; //公众号支付（H5）
        ...
        if ($arr['return_code'] != 'SUCCESS') {
            Log::record('QQ公众号支付网关错误，错误信息：' . $arr['return_msg'] . '，完整返回：' . json_encode($arr));
        ...
        Log::record('QQ公众号支付Pay，' . $arr['return_msg'] . '，完整返回：' . json_encode($arr) .
            "，原始返回：" . $ret);


        //调用SJAPI
        $this->callJsApi($arr['prepay_id']);

        //$return['amount'] = floatval($return['amount']) / $this->exchange; //转回以元为单位
        //$this->showQRcode($arr['code_url'], $return, 'qq');
    }

    private function callJsApi($tokenId)
    {
        $html = <<<EOT
<html>
<head>
