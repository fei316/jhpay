...
class JmaliwapController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $notifyurl = $this->_site . 'Pay_Jmaliwap_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Jmaliwap_callbackurl.html'; //返回通知
        $parameter = array(
            'code' => 'Jmaliwap', // 通道名称
            'title' => '支付宝H5',
            'exchange' => 1, // 金额比例
        ...
        $goodsname="团购商品";
       // $istype="1";
        $return_url =  $this->_site . 'Pay_Jmaliwap_callbackurl.html'; //返回通知
        $notify_url =  $this->_site . 'Pay_Jmaliwap_notifyurl.html'; //异步通知
...
return;
 ...
 protected function _createForm($url, $native){
    ...
    public function callbackurl()
    {
        ...
        if($pay_status <> 0){
            $this->EditMoney($_REQUEST["orderid"], 'Jmaliwap', 1);
        }else{
            exit("交易成功！");
        }
    ...
    public function notifyurl()
    {
       ...
       file_put_contents( dirname( __FILE__ ).'Jmaliwap_post.txt', var_export($_POST, true), FILE_APPEND );
       ...
       if($temps != $key)
       {$this->EditMoney($orderid, 'Jmaliwap', 0);
        	exit("OK");
        }
      }
