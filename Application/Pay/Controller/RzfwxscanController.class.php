...
class RzfwxscanController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $notifyurl = $this->_site . 'Pay_Rzfwxscan_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Rzfwxscan_callbackurl.html'; //返回通知
        ...
        $parameter = array(
            ...
            "productId" => "0108", //产品类型- 0119-支付宝扫码支付 0120-支付宝刷卡（反扫）0108-微信扫码支付 0113-微信刷卡（反扫）0112-公众号支付  0130 - 微信APP内支付  0131 - 支付宝APP内支付
    ...
    public function callbackurl()
    {
        ...
        if($pay_status <> 0){
            $this->EditMoney($_REQUEST["orderid"], 'Rzfwxscan', 1);
        }else{
            exit("error");
        }
    ...
    public function notifyurl()
    {
        ...
        if(!empty($parameter)){
            ...
            if($signature==$parameter['signature'])
            {
                ...
                {
                    $this->EditMoney($parameter['orderNo'], 'Rzfwxscan', 0);
                    //验证完成，
                    echo 'SUCCESS';
                }
                else
