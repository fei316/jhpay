...
class WjalipayController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $notifyurl = $this->_site . 'Pay_Wjalipay_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Wjalipay_callbackurl.html'; //返回通知
        $parameter = array(
            'code' => 'Wjalipay', // 通道名称
            'title' => '万家支付宝扫码',
            'exchange' => 1, // 金额比例
        ...
        $goodsname="团购商品";
        $istype="1";
        $return_url =  $this->_site . 'Pay_Wjalipay_callbackurl.html'; //返回通知
        $notify_url =  $this->_site . 'Pay_Wjalipay_notifyurl.html'; //异步通知
...
file_put_contents("Wjalipaypreturn.txt", date("Y-m-d H:i:s")." ".$ret."\r\n", FILE_APPEND );
...
//header("Location: $qrcode");
    ...
    public function callbackurl()
    {
        ...
        if($pay_status <> 0){
            $this->EditMoney($_REQUEST["orderid"], 'Wjalipay', 1);
        }else{
            exit("交易失败");
        }
    ...
    public function notifyurl()
    {
       ...
       file_put_contents( dirname( __FILE__ ).'Wjalipay_post.txt', var_export($_POST, true), FILE_APPEND );
       ...
       $key = $_REQUEST["key"];
       $info=M('ChannelAccount')->where(array('channel_id'=>225))->find();
	 	$channel=M('Channel')->where(array('id'=>225))->find();
        $token =$info['signkey'];
       $temps = md5($orderid . '+' .$orderuid . '+' .$platform_trade_no . '+' .$price . '+' .$realprice . '+' .$token);
       if($key== $temps)
       {$this->EditMoney($orderid, 'Wjalipay', 0);
        	exit("OK");
        }
      }
